...
export async function POST(request: NextRequest) {
  try {
    ...
    const mailOptions = {
      ...
      subject: `Job Application - ${body.jobTitle} - ${body.name}`,
      html: `
        <h3>Job Application</h3>
        <p><strong>Name:</strong> ${body.name}</p>
        <p><strong>Email:</strong> ${body.email}</p>
        <p><strong>Phone:</strong> ${body.phone}</p>
        <p><strong>Position:</strong> ${body.jobTitle}</p>
        <p><strong>Job ID:</strong> ${body.jobId}</p>
        <p><strong>Cover Letter:</strong> ${body.coverLetter || 'N/A'}</p>
        <p><strong>Note:</strong> Resume was uploaded but cannot be attached via JSON. Please contact the applicant for the resume.</p>
      `,
    };
    ...
    const thankYouOptions = {
      from: process.env.JOB_MAIL,
      to: body.email,
      subject: `Application Received - ${body.jobTitle} at Nigsoft`,
      html: getThankYouEmail(body.name, body.jobTitle)
    };
